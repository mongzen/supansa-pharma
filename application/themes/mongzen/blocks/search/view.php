...
?>
<form class="w-100 d-flex header-search" action="<?= $view->url($resultTarget) ?>" method="get"><?php
if (isset($title) && ($title !== '')) {
    ?><h3 class="d-flex items-center gap-1"><i class="bi bi-filter"></i><span><?= h($title) ?></span></h3><?php
...
?><input name="query" class="form-control ccm-search-block-text" type="text"
		value="<?= htmlentities($query, ENT_COMPAT, APP_CHARSET) ?>"
		placeholder="<?= t('ค้นหาสินค้าภายในร้าน...') ?>" /><?php
if (isset($buttonText) && ($buttonText !== '')) {
    ?> <div class="submit-wrapper"><input name="submit" type="submit" value="<?= h($buttonText) ?>"
			class="btn btn-secondary ccm-search-block-submit" /><i class="bi bi-search"></i></div><?php
